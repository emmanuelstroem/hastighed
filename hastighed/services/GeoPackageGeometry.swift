...
enum GeoPackageGeometry {
    ...
    static func extractWKB(from gpkgBlob: Data) throws -> Data {
        ...
        switch envelopeIndicator {
            ...
            case 4:
                envelopeSize = 64  // + Z + M
            default:
                envelopeSize = 0  // defensive fallback
        }
        let headerSize = 8 + envelopeSize  // 2 magic + 1 ver + 1 flags + 4 srsId + envelope
        guard gpkgBlob.count > headerSize else {
            throw GeoPackageGeometryError.malformed
        }
    ...
    static func parseWKB(_ wkb: Data) throws -> ParsedGeometry {
        ...
        func read<T>(_ type: T.Type) throws -> T {
            ...
            guard offset + size <= wkb.count else {
                throw GeoPackageGeometryError.malformed
            }
        ...
        let byteOrder = wkb[offset]
        offset += 1  // 0=big, 1=little
        ...
        func readDouble() throws -> Double {
            ...
            let val =
                littleEndian ? UInt64(littleEndian: v) : UInt64(bigEndian: v)
        ...
        switch baseType {
            ...
            case 5:  // MultiLineString (components are WKB LineStrings with own byte order and dims)
                ...
                for _ in 0..<n {
                    ...
                    guard offset + 5 <= wkb.count else {
                        throw GeoPackageGeometryError.malformed
                    }
                    let subByteOrder = wkb[offset]
                    offset += 1
                    let subLE = (subByteOrder == 1)
                    // read type
                    let tRaw: UInt32 = try read(UInt32.self)
                    let t =
                        subLE ? UInt32(littleEndian: tRaw) : UInt32(bigEndian: tRaw)
                    ...
                    guard subBase == 2 else {
                        throw GeoPackageGeometryError.unsupportedGeometryType(t)
                    }
                    // read point count
                    let nRaw: UInt32 = try read(UInt32.self)
                    let m =
                        subLE ? UInt32(littleEndian: nRaw) : UInt32(bigEndian: nRaw)
                    var coords: [CLLocationCoordinate2D] = []
                    ...
                    for _ in 0..<m {
                        ...
                        let x = Double(
                            bitPattern: subLE
                                ? UInt64(littleEndian: xBits)
                                : UInt64(bigEndian: xBits)
                        )
                        let y = Double(
                            bitPattern: subLE
                                ? UInt64(littleEndian: yBits)
                                : UInt64(bigEndian: yBits)
                        )
                        coords.append(
    ...
    static func distanceMeters(
        ...
        for i in 0..<(line.coordinates.count - 1) {
            ...
            let t =
                ab2 > 0 ? max(0, min(1, (ap.x * ab.x + ap.y * ab.y) / ab2)) : 0
