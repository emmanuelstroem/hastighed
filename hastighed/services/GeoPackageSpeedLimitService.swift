...
//        
    ...
    private func parseRawAndKmh(from tags: [String: String]) -> (rawValue: Int?, rawUnit: String?, kmh: Int?)? {
            // Minimal: extract a number from available fields; do not compute presentation unit here
        if let v = tags["maxspeed"], let n = Double(v.filter({ $0.isNumber || $0 == "." })) { let kmhVal = Int(n.rounded()); return (kmhVal, "km/h", kmhVal) }
        return nil
    }
    ...
    private func defaultSpeed(for highway: String) -> Int? {
        switch highway.lowercased() {
            ...
            case "tertiary", "unclassified": return 50
